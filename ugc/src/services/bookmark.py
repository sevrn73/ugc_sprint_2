...
async def get_bookmarks_list(
    ...
    data = await mongo.find(settings.MONGO_COLLECTION_BOOKMARK, {"user_id": user_id}, limit=limit, offset=offset)
...
async def get_bookmark(user_id: str, film_id: str) -> Optional[BookmarkModel]:
    ...
    data = await mongo.find_one(settings.MONGO_COLLECTION_BOOKMARK, {"user_id": user_id, "film_id": film_id})
    if not data:
...
async def remove_bookmark(user_id: str, film_id: str) -> None:
    ...
    await mongo.delete(settings.MONGO_COLLECTION_BOOKMARK, {"user_id": user_id, "film_id": film_id})
